	...
	for (int bi = 0; bi < bc; bi++) {
		...
		for (int ci = 0; ci < bun->count; ci++) {
			...
			if (con->is_synthetic) {
				...
				double soft = sol_joints[con->solver_idx].softness;
				if (soft > 0.0) {
...
static int ldl_cache_rebuild_blocks(LDL_Cache* c, WorldInternal* w, int island_idx, SolverJoint* sol_joints, int joint_count)
{
	afree(c->constraints);
	afree(c->bundles);
	afree(c->body_remap);
	...
	while (ji >= 0) {
		for (int i = 0; i < joint_count; i++) {
			if (sol_joints[i].joint_idx == ji) {
				...
				// LDL is strictly for equality constraints. Any bounded DOF
				// (limits, motors, unilateral types) makes the whole joint
				// fall to PGS so all its DOFs stay coupled in one loop.
				if (joint_internal_has_limits(jj)) break;
...
static void ldl_island_solve(LDL_Cache* c, WorldInternal* w, SolverJoint* sol_joints, int joint_count, float sub_dt, LDL_CompBody* comp_bodies)
{
	...
	for (int i = 0; i < jc; i++) {
		...
		SolverJoint* sj = &sol_joints[con->solver_idx];
		for (int d = 0; d < con->dof; d++) {
			if (sj->softness > 0.0f) sj->lambda[d] += (float)vel_lambda[oi + d];
