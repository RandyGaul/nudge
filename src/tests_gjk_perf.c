...
static void test_gjk_known_distances()
{
	...
	TEST_BEGIN("hull-hull (box via hull)");
	{
		...
		v3 s1[8], s2[8]; float soa1[24], soa2[24];
		GJK_Shape ga = gjk_hull_scaled(ub, V3(0,0,0), id, V3(1,1,1), s1, soa1);
		GJK_Shape gb = gjk_hull_scaled(ub, V3(4,0,0), id, V3(1,1,1), s2, soa2);
	...
	{
		const Hull* ub = hull_unit_box();
		v3 s[8]; float ssoa[24];
		GJK_Shape pt = gjk_sphere(V3(2,2,2), 0);
		GJK_Shape bx = gjk_hull_scaled(ub, V3(0,0,0), id, V3(1,1,1), s, ssoa);
...
static void test_gjk_bf_box_box()
{
	...
	for (int c = 0; c < BF_NCASES; c++) {
		...
		// gjk_hull path
		v3 s1[8], s2[8]; float soa1[24], soa2[24];
		GJK_Shape ga = gjk_hull_scaled(ub, posA, rA, heA, s1, soa1);
		GJK_Shape gb = gjk_hull_scaled(ub, posB, rB, heB, s2, soa2);
...
static void test_gjk_bf_segment_hull()
{
	...
	for (int c = 0; c < BF_NCASES; c++) {
		...
		v3 sb[8]; float soasb[24];
		GJK_Shape sa = gjk_capsule(segP, segQ, 0);
		GJK_Shape sbx = gjk_hull_scaled(ub, boxPos, rB, boxHE, sb, soasb);
...
static void test_gjk_bf_sphere_box()
{
	...
	for (int c = 0; c < BF_NCASES; c++) {
		...
		v3 sb[8]; float soasb[24];
		GJK_Shape sbh = gjk_hull_scaled(ub, boxPos, rB, boxHE, sb, soasb);
...
static PerfRow perf_hull_hull(int n_target)
{
	...
	GJK_Shape a[PERF_CONFIGS], b[PERF_CONFIGS];
	v3 sca[PERF_CONFIGS][1024], scb[PERF_CONFIGS][1024]; float soac_a[PERF_CONFIGS][3072], soac_b[PERF_CONFIGS][3072];
	for (int c = 0; c < PERF_CONFIGS; c++) {
		float sep = 0.5f + (float)(c % 4) * 2.5f;
		float sc = 0.5f + (float)(c / 4 % 4) * 2.0f;
		a[c] = gjk_hull_scaled(ha, V3(0,0,0), gjk_perf_random_quat(), V3(sc, sc*0.7f, sc*0.5f), sca[c], soac_a[c]);
		b[c] = gjk_hull_scaled(hb, V3(sep + sc*3, 0, 0), gjk_perf_random_quat(), V3(sc*0.8f, sc, sc*0.6f), scb[c], soac_b[c]);
	}
...
static void test_gjk_bf_hull_hull(int n_target)
{
	...
	for (int c = 0; c < ncases; c++) {
		...
		v3 sa[1024], sb[1024]; float soaa[3072], soab[3072];
		GJK_Shape ga = gjk_hull_scaled(ha, posA, rA, heA, sa, soaa);
		GJK_Shape gb = gjk_hull_scaled(hb, posB, rB, heB, sb, soab);
		double err = fabs((double)gjk_distance(ga, gb).distance - ref.distance);
		if (err > max_err) max_err = err;
