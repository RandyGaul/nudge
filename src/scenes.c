...
// ---------------------------------------------------------------------------
// Scene: Soft Bodies -- rope + cloth + jelly test bed.
// ---------------------------------------------------------------------------
static void scene_soft_bodies_setup()
{
	add_floor();

	// 1) Pinned rope, 16 nodes, spacing 0.3, hanging from y=6.
	{
		...
		int first = -1;
		for (int i = 0; i < N; i++) {
			v3 p = V3(anchor.x, anchor.y - 0.3f * i, anchor.z);
			int node = soft_body_add_node(g_world, rope, p, 0.5f);
			if (i == 0) first = node;
			if (i > 0) soft_body_add_link(g_world, rope, node - 1, node, -1.0f, (SpringParams){0});
		}
		soft_body_build(g_world, rope);
		soft_body_pin_static(g_world, rope, first, anchor);
		apush(g_soft_bodies, rope);
	}

	// 2) Cross-braced 5x5 cloth, pinned at top-left and top-right corners.
	{
		...
		int W = 5;
		float spacing = 0.35f;
		v3 origin = V3(-1.0f, 6.0f, 0.0f);
		int nodes[5][5];
		for (int j = 0; j < W; j++) {
			for (int i = 0; i < W; i++) {
				v3 p = V3(origin.x + i * spacing, origin.y, origin.z - j * spacing);
				nodes[j][i] = soft_body_add_node(g_world, cloth, p, 0.2f);
			}
		}
		// Grid edges (structural)
		for (int j = 0; j < W; j++) {
			for (int i = 0; i < W; i++) {
				if (i + 1 < W) soft_body_add_link(g_world, cloth, nodes[j][i], nodes[j][i+1], -1.0f, (SpringParams){0});
				if (j + 1 < W) soft_body_add_link(g_world, cloth, nodes[j][i], nodes[j+1][i], -1.0f, (SpringParams){0});
			}
		}
		// Diagonal shear braces
		for (int j = 0; j + 1 < W; j++) {
			for (int i = 0; i + 1 < W; i++) {
				soft_body_add_link(g_world, cloth, nodes[j][i],   nodes[j+1][i+1], -1.0f, (SpringParams){0});
				soft_body_add_link(g_world, cloth, nodes[j][i+1], nodes[j+1][i],   -1.0f, (SpringParams){0});
			}
		}
		soft_body_build(g_world, cloth);
		soft_body_pin_static(g_world, cloth, nodes[0][0],     V3(origin.x,                         origin.y, origin.z));
		soft_body_pin_static(g_world, cloth, nodes[0][W-1],   V3(origin.x + (W-1)*spacing,         origin.y, origin.z));
		apush(g_soft_bodies, cloth);
	}

	// 3) Free-falling 4x4x4 jelly lattice with soft spring links (wobbly).
	{
		...
		int D = 4;
		float spacing = 0.25f;
		v3 origin = V3(4.0f, 4.0f, 0.0f);
		int nodes[4][4][4];
		for (int z = 0; z < D; z++)
			for (int y = 0; y < D; y++)
				for (int x = 0; x < D; x++) {
					v3 p = V3(origin.x + x * spacing, origin.y + y * spacing, origin.z + z * spacing);
					nodes[z][y][x] = soft_body_add_node(g_world, jelly, p, 0.1f);
				}
		// Grid edges along x, y, z
		for (int z = 0; z < D; z++)
			for (int y = 0; y < D; y++)
				for (int x = 0; x < D; x++) {
					if (x + 1 < D) soft_body_add_link(g_world, jelly, nodes[z][y][x], nodes[z][y][x+1], -1.0f, (SpringParams){0});
					if (y + 1 < D) soft_body_add_link(g_world, jelly, nodes[z][y][x], nodes[z][y+1][x], -1.0f, (SpringParams){0});
					if (z + 1 < D) soft_body_add_link(g_world, jelly, nodes[z][y][x], nodes[z+1][y][x], -1.0f, (SpringParams){0});
				}
		// Face diagonals on each 1x1x1 cell, 3 axes -> 6 diagonals per cell
		for (int z = 0; z + 1 < D; z++)
			for (int y = 0; y + 1 < D; y++)
				for (int x = 0; x + 1 < D; x++) {
					// XY face
					soft_body_add_link(g_world, jelly, nodes[z][y][x],   nodes[z][y+1][x+1], -1.0f, (SpringParams){0});
					soft_body_add_link(g_world, jelly, nodes[z][y+1][x], nodes[z][y][x+1],   -1.0f, (SpringParams){0});
					// YZ face
					soft_body_add_link(g_world, jelly, nodes[z][y][x],   nodes[z+1][y+1][x], -1.0f, (SpringParams){0});
					soft_body_add_link(g_world, jelly, nodes[z+1][y][x], nodes[z][y+1][x],   -1.0f, (SpringParams){0});
					// XZ face
					soft_body_add_link(g_world, jelly, nodes[z][y][x],   nodes[z+1][y][x+1], -1.0f, (SpringParams){0});
					soft_body_add_link(g_world, jelly, nodes[z+1][y][x], nodes[z][y][x+1],   -1.0f, (SpringParams){0});
				}
		soft_body_build(g_world, jelly);
		apush(g_soft_bodies, jelly);
	}
}

// ---------------------------------------------------------------------------
// Scene: Soft Ball -- geodesic squishy sphere rolling down stairs.
//
// Outer shell: subdivided icosphere (42 vertices, 120 edges). Inner shell:
// 12-vertex icosahedron at half radius (30 edges). Each outer vertex links
// radially to its nearest inner vertex (42 outer-inner links). Inner shell
// has 6 antipodal body diagonals for stiffness.
//
// Total: 54 nodes, 120 + 30 + 42 + 6 = 198 links. K is 198x198 dense.
// Factorization per substep is a few milliseconds in Debug, sub-millisecond
// in Release -- easily real-time for a gameplay-scale squishy ball.
// ---------------------------------------------------------------------------
static void scene_soft_ball_setup()
{
	add_floor();

	// Static stairs descending in +X.
	for (int i = 0; i < 6; i++) {
		float x = 2.0f + i * 1.2f;
		float y = 3.2f - i * 0.5f;
		Body step = create_body(g_world, (BodyParams){
			.position = V3(x, y, 0),
			.rotation = quat_identity(),
			.mass = 0,
			.friction = 0.6f,
		});
		body_add_shape(g_world, step, (ShapeParams){
			.type = SHAPE_BOX,
			.box.half_extents = V3(0.7f, 0.1f, 2.0f),
		});
		v3 tint = V3(0.4f + i * 0.06f, 0.45f, 0.55f);
		apush(g_draw_list, ((DrawEntry){ step, MESH_BOX, V3(0.7f, 0.1f, 2.0f), tint }));
	}

	// Icosahedron base: 12 unit-sphere vertices. phi = (1+sqrt(5))/2.
	float phi = (1.0f + sqrtf(5.0f)) * 0.5f;
	float nrm = 1.0f / sqrtf(1.0f + phi * phi);
	float aa = phi * nrm, bb = 1.0f * nrm;
	v3 ico[12] = {
		V3(-bb,  aa,  0.0f), V3( bb,  aa,  0.0f), V3(-bb, -aa,  0.0f), V3( bb, -aa,  0.0f),
		V3(0.0f, -bb,  aa), V3(0.0f,  bb,  aa), V3(0.0f, -bb, -aa), V3(0.0f,  bb, -aa),
		V3( aa, 0.0f, -bb), V3( aa, 0.0f,  bb), V3(-aa, 0.0f, -bb), V3(-aa, 0.0f,  bb),
	};
	int ico_tris[20][3] = {
	...
	int ico_antipode[6][2] = { {0, 3}, {1, 2}, {4, 7}, {5, 6}, {8, 11}, {9, 10} };

	// Subdivide the icosahedron once: split each edge at its midpoint (normalized
	// to the unit sphere), and each triangle into 4 smaller triangles.
	v3 sub_verts[42];
	int sub_tris[80][3];
	int mid[12][12];
	for (int i = 0; i < 12; i++) for (int j = 0; j < 12; j++) mid[i][j] = -1;
	for (int i = 0; i < 12; i++) sub_verts[i] = ico[i];
	int vc = 12, tc = 0;
	for (int t = 0; t < 20; t++) {
		int a = ico_tris[t][0], b = ico_tris[t][1], c = ico_tris[t][2];
		int pair[3][2] = { {a, b}, {b, c}, {c, a} };
		int m_idx[3];
		for (int e = 0; e < 3; e++) {
			int u = pair[e][0], v = pair[e][1];
			int lo = u < v ? u : v, hi = u < v ? v : u;
			int m = mid[lo][hi];
			if (m < 0) {
				m = vc;
				sub_verts[vc++] = v3_norm(add(sub_verts[u], sub_verts[v]));
				mid[lo][hi] = m;
			}
			m_idx[e] = m;
		}
		// Four new triangles.
		sub_tris[tc][0] = a;        sub_tris[tc][1] = m_idx[0]; sub_tris[tc][2] = m_idx[2]; tc++;
		sub_tris[tc][0] = b;        sub_tris[tc][1] = m_idx[1]; sub_tris[tc][2] = m_idx[0]; tc++;
		sub_tris[tc][0] = c;        sub_tris[tc][1] = m_idx[2]; sub_tris[tc][2] = m_idx[1]; tc++;
		sub_tris[tc][0] = m_idx[0]; sub_tris[tc][1] = m_idx[1]; sub_tris[tc][2] = m_idx[2]; tc++;
	}
	...
	// Outer shell: 42 subdivided-icosphere nodes.
	int outer[42];
	for (int i = 0; i < 42; i++) {
		outer[i] = soft_body_add_node(g_world, ball, add(center_pos, scale(sub_verts[i], outer_radius)), 0.08f);
	}
	// Inner shell: 12 icosahedron nodes at half radius (heavier, keeps centre of mass stable).
	int inner[12];
	for (int i = 0; i < 12; i++) {
		inner[i] = soft_body_add_node(g_world, ball, add(center_pos, scale(ico[i], inner_radius)), 0.2f);
	}

	// Outer surface edges (120) from the subdivided triangulation.
	int have_outer[42][42] = {{0}};
	for (int t = 0; t < 80; t++) {
		int ti[3] = { sub_tris[t][0], sub_tris[t][1], sub_tris[t][2] };
		for (int k = 0; k < 3; k++) {
			int u = ti[k], v = ti[(k + 1) % 3];
			int lo = u < v ? u : v, hi = u < v ? v : u;
			if (have_outer[lo][hi]) continue;
			have_outer[lo][hi] = 1;
			soft_body_add_link(g_world, ball, outer[lo], outer[hi], -1.0f, (SpringParams){0});
		}
	}
	// Inner-shell edges (30) from the icosahedron triangulation.
	int have_inner[12][12] = {{0}};
	for (int t = 0; t < 20; t++) {
		int ti[3] = { ico_tris[t][0], ico_tris[t][1], ico_tris[t][2] };
		for (int k = 0; k < 3; k++) {
			int u = ti[k], v = ti[(k + 1) % 3];
			int lo = u < v ? u : v, hi = u < v ? v : u;
			if (have_inner[lo][hi]) continue;
			have_inner[lo][hi] = 1;
			soft_body_add_link(g_world, ball, inner[lo], inner[hi], -1.0f, (SpringParams){0});
		}
	}
	// Outer -> nearest inner (42 radial links through the mesh interior).
	for (int i = 0; i < 42; i++) {
		int best = 0;
		float best_dot = -2.0f;
		for (int j = 0; j < 12; j++) {
			float d = dot(sub_verts[i], ico[j]); // both unit vectors; max dot == closest on unit sphere
			if (d > best_dot) { best_dot = d; best = j; }
		}
		soft_body_add_link(g_world, ball, outer[i], inner[best], -1.0f, (SpringParams){0});
	}
	// Body diagonals on inner shell (6 antipodal pairs).
	for (int i = 0; i < 6; i++) {
		soft_body_add_link(g_world, ball, inner[ico_antipode[i][0]], inner[ico_antipode[i][1]], -1.0f, (SpringParams){0});
	}

	soft_body_build(g_world, ball);
	apush(g_soft_bodies, ball);
}
