...
{
	printf(
		"Viewer commands (local, reads engine memory via ReadProcessMemory):\n"
		"  get <path>                          print struct at path\n"
		"  table <path> [fields...]            array as columns\n"
		"  summary                             scene overview (bodies, contacts, islands, perf)\n"
		"  contacts <body_idx>                 all solver manifolds + contacts for a body\n"
		"  warm [body_idx]                     warm cache entries with body pair keys\n"
		"  filter <path> <field> <op> <val>    find matches (ops: > < >= <= == !=)\n"
		"  snap [path]                         save state snapshot for diff\n"
		"  diff                                compare current state with snapshot\n"
		"  stable [threshold] [frames]         check all bodies settled (mechanical pass/fail)\n"
		"  replay <file>                       play back a repro script file\n"
		"  raw <addr> [len]                    hex dump at address\n"
		"  help                                this help\n"
		"\n"
		"Engine commands (sent to engine via TCP — run 'help' in engine for full list):\n"
		"  pause | step [n] | run [n]          simulation control\n"
		"  scene <name> | scenes | restart     scene management\n"
		"  push <idx> <f> [r]                  impulse at body (generates torque)\n"
		"  drag <idx> <local> <target>         mouse-like soft spring drag\n"
		"  dragto <target> | release           update/end drag\n"
		"  highlight <idx> <color> | unhighlight <idx|all>  visual tinting\n"
		"  label <text> | slow <factor>        overlay text, time scale\n"
		"  playrecording                       replay mouse_recording.bin\n"
		"  npviz                               enter NP Viz mode\n"
		"  npvset <A|B> <kind> <pos> <rot> <radius> <hh> <he>  set NP Viz shape\n"
		"  info | types                        engine status, type reflection\n"
		"\n"
		"Paths (for get/table/filter/snap):\n"
		"  body_state/<n>  body_hot/<n>  body_cold/<n>/shapes/0  islands/<n>\n"
		"  bvh_dynamic/nodes/<n>  warm_cache/<n>  dbg_solver_manifolds/<n>\n"
	);
}
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		if (strcmp(cmd, "get") == 0) cmd_get(rest);
		         ...
		         strcmp(cmd, "npviz") == 0 || strcmp(cmd, "npvset") == 0 ||
		         strcmp(cmd, "drag") == 0 ||
		         ...
		         strcmp(cmd, "break-filter") == 0 || strcmp(cmd, "np-debug") == 0) {
			driver_cmd(line);
